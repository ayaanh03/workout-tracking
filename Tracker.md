...
26. **Permanent weekly-template change (effective W6, 2026-05-27):** Hyp A moved **Tue → Wed**; the midweek Z1 easy run moved **Wed → Thu**. Tue is now threshold-only — decouples the lift from the quality run, resolving the recurring Adj #16 same-day lift/run friction (cleaner T-pace signal). Thu is no longer a flex-rest day (user accepted dropping the second rest day); **revert Thu to rest only on yellow/red mornings, §11.C**. Hyp A stays upper-dominant + soleus, so the Wed slot doesn't cost Fri's legs; Sat double (long run AM + Hyp B PM) unchanged. Updated in program.md §2.A / §2.C / §3.A / §3.C / §4.C–E / §5 / §6 / §12 and CLAUDE.md quick-ref. **week-ahead.json still reflects the old W6 shape (Wed easy / Thu rest) — will refresh on the next morning brief.**
...
|W6|✓ Z1 + 4×20s strides 5-25 (cadence 180/178/196/180; Adj #24 mostly held, rep 4 textbook)|—|—|—|—|—|in progress|