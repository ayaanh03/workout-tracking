...
- `history/` — **everything historical for the tracker side, one append-only file per stream; live files hold the current phase, trimmed by the ~40 KB backstop:** `history/adjustments.md` (the numbered ledger, **Adj #347+** — #164–#253 rotated to `archive/adjustments-164-253.md` 2026-08-12, Adj #272; #254–#282 rotated to `archive/adjustments-254-282.md` 2026-08-19, Adj #295; #283–#308 rotated to `archive/adjustments-283-308.md` 2026-08-24, Adj #311; #309–#346 rotated to `archive/adjustments-309-346.md` 2026-09-07, Adj #347), `history/lifting-log.md` (**W18+** — W13–W16 rotated to `archive/lifting-log-W13-W16.md`, same pass; W17 rotated to `archive/lifting-log-W17.md` 2026-08-26, Adj #319), `history/cardio-log.md` (**W17+** — W13–W16 plus the 5K/3K TT detail entries rotated to `archive/cardio-log-W13-W16.md` 2026-08-26, Adj #326), `history/si-log.md`, `history/weekly-summary.md` (completed weeks), `history/incline-bb.md` (current meso, whole). Every live file's append point is **end of file**. Completed phases live verbatim in `history/archive/`. `Tracker.md` cites an Adj # or week; to resolve a cited Adj #N: `grep -rn '^N\. ' history/` — **never read a history file in full.**
...
6. **Mark today in `Tracker.md ## Weekly Summary`** (`✓ {Session} {date}` in the current week's row — this is the only place the in-progress week is tracked). **SIZE RULE — each cell is `✓ {Session} {date} ({key metric}; Adj #)` — one line, no multi-paragraph narratives. Full session detail lives in `history/`.** **When a week closes** (its last session logs), append the completed row to the end of `history/weekly-summary.md` and start the new week's row in `Tracker.md`.
7. **Sync the lean `Tracker.md`** so it reflects the new current state — this is the only writing `Tracker.md` itself gets:
   - **`## Current Working Loads`**: update any load that bumped, held, or changed status. Update "Last verified" to today and rewrite the Note column with what just happened. **SIZE RULE — the Note column is ONE line: the forward-looking action (gate, revert rule, hold condition) + Adj # citation. Session narratives and historical context belong in `history/adjustments.md`.**
   - **`## Active Flags`**: add/remove/reword bullets so the list matches what's actually open after today (resolved flags drop off; new ones get added, citing the new Adj #). **SIZE RULE — each flag is ONE bullet, 1–3 lines max: the actionable item + Adj # citation. Session-by-session rationale, historical diagnosis chains, and superseded reads belong in `history/adjustments.md`, not here. If a flag needs more than 3 lines to state, the extra detail is history — move it to the ledger entry and cite the Adj #. Completed session reports are NEVER flags.** Tennis elbow and R patellar tendon watches are RETIRED — do not re-add them as flags (clean through the Build; resume only if the athlete reports a new symptom).
